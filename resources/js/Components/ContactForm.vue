  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          <div class="pr-6">
            ...
            <div class="mt-6 text-sm text-gray-600 space-y-2">
              ...
              <div v-if="(contact.phones || []).length"><strong>Teléfono:</strong> <span v-for="(p,i) in contact.phones" :key="i">{{ p }}<span v-if="i+1 < contact.phones.length">, </span></span></div>
              <div v-if="(contact.emails || []).length"><strong>Email:</strong> <span v-for="(e,i) in contact.emails" :key="i">{{ e }}<span v-if="i+1 < contact.emails.length">, </span></span></div>
            </div>
